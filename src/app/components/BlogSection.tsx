        ...
        {blogs.map((blog) => (
          <div key={blog.id} className="flex flex-col items-start">
            ...
            <div className="w-full h-[300px] relative rounded-lg shadow-md bg-white p-2">
              <Image
                src={blog.img}
                alt={`Image for ${blog.title}`}  // Improved alt text for accessibility
                layout="fill"
                objectFit="cover"
            ...
            <div className="flex items-center space-x-2 mt-4">
              {/* Pen Icon */}
              <Image src="/images/vector2.png" alt="Pen icon" width={20} height={20} />
            ...
            {/* Blog Description with Text Truncation */}
            <p className="text-gray-600 mt-2 line-clamp-3">{blog.description}</p>

            {/* Read More Link */}
            <Link href={`/blog/${blog.id}`}>  {/* Dynamic Link to individual blog */}
              <button
